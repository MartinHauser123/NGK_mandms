...
struct place_t
{
	...
	json_io(JSON_IO &io)
	{
		io
			...
			json_dto::mandatory("latitude", m_lat) & json_dto::mandatory("longitude", m_lon);
	}
...
struct weatherReport_t
{
	...
	weatherReport_t(
		...
		float temperature,
		int humidity)
		: m_id{std::move(id)}, m_date{std::move(date)}, m_time{std::move(time)}, m_place{std::move(place)}, 
		m_temperature{std::move(temperature)}, m_humidity{std::move(humidity)}
	{
	}
	...
	json_io(JSON_IO &io)
	{
		io
			...
			json_dto::mandatory("date", m_date) & json_dto::mandatory("time", m_time) 
			& json_dto::mandatory("place", m_place) & json_dto::mandatory("temperature", m_temperature) 
...
public:
		...
		const restinio::request_handle_t &req, rr::route_params_t params)
	{
		...
		if (0 != reportNum && reportNum <= m_weatherReport_t.size())
		{
			...
			resp.set_body(
				"Delete weatherReport #" + std::to_string(reportNum) + ": " + report.m_place.m_placeName + "[" + report.m_date + ", " + report.m_time + "]\n");
	...
	{
		const auto methods = "OPTIONS, GET, POST, DELETE, PUT";
		auto resp = init_resp(req->create_response());
...
auto server_handler(weatherStation_t &weatherReport_collection)
{
	...
	router->add_handler(restinio::http_method_options(), "/", by(&weatherReport_handler_t::options));
	...
	router->add_handler(restinio::http_method_options(), R"(/:reportNum(\d+))", by(&weatherReport_handler_t::options));
